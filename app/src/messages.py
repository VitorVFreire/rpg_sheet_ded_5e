...
class Messages:
    def __init__(self, room_id = None, offset = None, limit = None):
        ...
        self.__total_number_messages = 0
        self.__messages = {
            'room_id': self.__room_id,
            'offset': self.__offset,
            'limit': self.__limit,
            'total_number_messages': self.__total_number_messages,
            'messages': []
        }
    ...
    def messages(self):
        ...
        self.__messages['limit'] = self.__limit
        self.__messages['total_number_messages'] = self.__total_number_messages
    ...
    async def load_messages(self):
        try:   
            if self.__room_id:
                query = """
                SELECT m.message_id, m.message, m.messagetime AS time, COALESCE(m.character_id, m.user_id) AS id, COALESCE(c.character_name, u.user_name) AS name, (SELECT COUNT(*) FROM message m WHERE m.room_id = %s) AS count
                FROM message m
                ...
                """
                parameters = (self.__room_id, self.__room_id, self.__limit, self.__offset,)
                db = Db()
                ...
                if result:
                    ...
                    self.__total_number_messages = result[0][5]
                    return True
            return False
